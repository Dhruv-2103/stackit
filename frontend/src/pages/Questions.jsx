...
import { Search, Filter, ArrowUpDown, ChevronUp, ChevronDown, Eye, MessageSquare, Check, Tags, Plus, X, LogIn } from 'lucide-react'
...
const Questions = () => {
  ...
  const { questions, searchQuery, tags, getQuestions, getTags, upvoteQuestion, downvoteQuestion, isLoading } = useQuestionStore()
  ...
  const handleAskQuestion = () => {
    if (!user) {
      setLoginConfirm({ show: true, action: 'create a question' })
      return
    }
    navigate('/ask')
  }
  ...
  const handleVote = async (questionId, type) => {
    if (!user) {
      setLoginConfirm({ show: true, action: 'vote on questions' })
      return
    }
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
            ...
            <div className={`flex-1 transform transition-all duration-1000 lg:delay-300 ${isLoaded ? 'translate-x-0 opacity-100' : 'lg:translate-x-full opacity-0'
              ...
              {/* Ask Question button is now in the header */}
      ...
      <Confirm
        ...
        type="info"
        message={`Please sign in to ${loginConfirm.action}.`}
        confirmText="Sign In"
        cancelText="Cancel"
      />
    </>
